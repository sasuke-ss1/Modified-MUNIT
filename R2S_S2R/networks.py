...
# -*- coding: utf-8 -*-
"""
Created on Tue May 31 19:04:27 2022
...
import numpy as np
import functools
import MUNIT.networks as munit
...
def define_G(input_dims, params, device, pretrained = True, path = None):
    netG = munit.AdaINGen(input_dims, params)
    netG.to(device)
    if pretrained:
        assert path, "Please Give Generator Weights path"
...
def define_D(input_dim, params, device, pretrained = True, path = None):
    netD = munit.MsImageDis(input_dim, params)
    netD.to(device)
    if pretrained:
        assert path, "Please Give Discriminator Weights path"
...
class GaussianNoiseLayer(nn.Module):
    ...
    def forward(self, x):
        if self.training == False:
            return x
        noise = (torch.randn(x.size()).cuda(x.data.get_device()) - 0.5) / 10.0
        return x+noise